...
import { Add, ArrowDown, ArrowUp, Close } from "../components/icons/icons";
...
const Items = () => {
  ...
  const handleEndConcert = async () => {
    ...
    data.append(
      "data",
      JSON.stringify(fetchTask.data.length > 0 ? fetchTask.data[0]?.tasks : [])
    );
  ...
  return (
    <div className={css.itemContainer}>
      ...
      {showCollection && (
        <div className={css.itemContainer_collection}>
          <div
            onClick={() => setShowCollection(false)}
            className={css.itemContainer_collection_close}
          >
            <Close />
          </div>
          <Link to={"/collections"}>Collections</Link>
          <Cards
            data={fetchCollection.data}
            optionShow={false}
            sidePare={false}
          />
        </div>
      ...
      <div className={css.itemContainer_collection_computer}>
        <Link to={"/collections"}>Collections</Link>
        <Cards
          className={css.itemContainer_collection_computer_cardCollection}
          data={fetchCollection.data}
          optionShow={false}
          sidePare={true}
        />
      ...
      <div className={css.itemContainer_content}>
        ...
        <div className={css.itemContainer_content_row}>
          {fetchTask.data.length > 0 &&
            fetchTask.data[0]?.tasks.map((task) => {
              return (
                <Todo
                  hanlderComplete={handleEndConcert}
                  fetchTask={fetchTask}
                  key={task.id}
                  id={task.id}
                  task={task.name}
        ...
        <div className={css.itemContainer_additem}>
          <div
            ...
            className={`${css.itemContainer_additem_btn} ${
              showAddFrom ? css.itemContainer_additem_btn_active : null
            }`}
          >
            <h3>New Task</h3>
            <Add />
          ...
          <div
            className={`${css.itemContainer_additem_form} ${
              showAddFrom && css.itemContainer_additem_form_active
            }`}
          >
            <Formik
              initialValues={{
              ...
              onSubmit={(values) => {
                ...
                setTimeout(() => {
                  if (!addTask.loading) {
                    dispatch(
                      FetchDataTask({ token: ctx.token, id: params.id })
                    );
                  }
                  setShowAddFrom(false);
                }, 100);
